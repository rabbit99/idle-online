...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      const profile = Array.isArray(data) ? (data[0] as ProfileRow) : (data as ProfileRow | null);
      if (error) {
        setLog((prev: LogEntry[]) => [{ type: "warn", message: `載入存檔失敗：${error.message}` }, ...prev]);
      } else if (profile) {
        setStamina(profile.stamina ?? 20);
        setGold(profile.gold ?? 0);
        setCheckedIn(Boolean(profile.checked_in));
        setLog((prev: LogEntry[]) => [{ type: "system", message: "已載入雲端存檔" }, ...prev]);
      } else {
        setLog((prev: LogEntry[]) => [{ type: "warn", message: "載入存檔失敗：未取得資料" }, ...prev]);
      }
      setProfileLoading(false);
      setIsHydrated(true);
    };
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = window.setTimeout(async () => {
      ...
      if (error) {
        setLog((prev: LogEntry[]) => [{ type: "warn", message: `儲存失敗：${error.message}` }, ...prev]);
      }
    }, 800);
  ...
  const handleCheckIn = () => {
    ...
    const run = async () => {
      ...
      const profile = Array.isArray(data) ? (data[0] as ProfileRow) : (data as ProfileRow | null);
      if (error) {
        setLog((prev: LogEntry[]) => [{ type: "warn", message: `打卡失敗：${error.message}` }, ...prev]);
        return;
      }
      if (profile) {
        setStamina(profile.stamina ?? 20);
      ...
      setLog((prev: LogEntry[]) => [{ type: "checkin", message: "完成每日打卡，體力 +20" }, ...prev]);
    };
  ...
  const handleWork = (job: Job) => {
    if (stamina < job.staminaCost) {
      setLog((prev: LogEntry[]) => [{ type: "warn", message: "體力不足，無法開始工作" }, ...prev]);
      return;
    }
    setStamina((prev: number) => prev - job.staminaCost);
    setGold((prev: number) => prev + job.rewardGold);
    setLog((prev: LogEntry[]) => [{ type: "work", message: `完成 ${job.title}，獲得 ${job.rewardGold} 金幣` }, ...prev]);
  };
